...
/* do_lookup_x的实用函数。调用者使用undef_name、ref、version、flags和type_class调用，
	并将它们作为前五个参数传递。然后调用者计算sym、symidx、strtab和map，并将它们作为下一个四个参数传递。
	最后，调用者传入versioned_sym和num_versions，这些参数在检查过程中由check_match修改。*/
				...
				int *const num_versions)
{
	...
	if (__glibc_unlikely((sym->st_value == 0 /* No value.  */
						 ...
						 elf_machine_sym_no_match(sym) || (type_class & (sym->st_shndx == SHN_UNDEF))))
		return NULL;
...
#define ALLOWED_STT \
	((1 << STT_NOTYPE) | (1 << STT_OBJECT) | (1 << STT_FUNC) | (1 << STT_COMMON) | (1 << STT_TLS) | (1 << STT_GNU_IFUNC))
...
is_nodelete(struct link_map *map, int flags)
{
	...
	return map->l_nodelete_active || ((flags & DL_LOOKUP_FOR_RELOCATE) && map->l_nodelete_pending);
}

/* Utility function for do_lookup_x. Lookup an STB_GNU_UNIQUE symbol
   in the unique symbol table, creating a new entry if necessary.
   Return the matching symbol in RESULT.  */
/* do_lookup_x的实用函数。在唯一符号表中查找STB_GNU_UNIQUE符号，如果需要，创建一个新条目。
	将匹配的符号返回到RESULT中。*/
static void
				 ...
				 int flags)
{
	...
	if (entries != NULL)
	{
		...
		while (1)
		{
			if (entries[idx].hashval == new_hash && strcmp(entries[idx].name, undef_name) == 0)
			{
		...
		if (size * 3 <= tab->n_elements * 4)
		{
			...
			struct unique_sym *newentries = calloc(sizeof(struct unique_sym), newsize);
			if (newentries == NULL)
			{
			nomem:
				__rtld_lock_unlock_recursive(tab->lock);
...
/* 查找函数的内部部分。如果找到了符号，则返回值>0，如果没有找到，
	则返回值0，如果发生了一些不好的事情，则返回值<0。*/
static int
				...
				struct link_map *skip, int type_class, struct link_map *undef_map)
{
	...
	// 确保在继续之前读取值。否则，我们可能会使用r_list指向初始作用域，
	// 而r_nlist是调整大小后的值。这是dl-open.c中唯一一个没有受GSCOPE保护的路径。
	// 这里的读屏障可能太昂贵了。
	__asm volatile("" : "+r"(n), "+m"(scope->r_list));
	struct link_map **list = scope->r_list;	// link_map列表

	do
	{
		const struct link_map *map = list[i]->l_real;	// 真正的link_map

		/* Here come the extra test needed for `_dl_lookup_symbol_skip'.  */
		// 这里是_dl_lookup_symbol_skip所需的额外测试。
		if (map == skip)
			continue;
		...
		// 此映射的表。
		const ElfW(Sym) *symtab = (const void *)D_PTR(map, l_info[DT_SYMTAB]);	// link_map对应的链接库的符号表
				...
				if (bucket != 0)
				{
					...
					do
						if (((*hasharr ^ new_hash) >> 1) == 0)
						{
							...
							sym = check_match(undef_name, ref, version, flags,	// 比对函数名、版本信息
											  type_class, &symtab[symidx], symidx,
											  strtab, map, &versioned_sym,
											  &num_versions);
							if (sym != NULL)	// 若找到符号，直接跳出循环
								goto found_it;
						}
					while ((*hasharr++ & 1u) == 0);
				}
		...
		else
		{
			...
			// 使用旧的SysV风格哈希表。在此对象的符号表中搜索适当的哈希桶，
			// 以查找相同符号名的定义。
	   ...
	   no possible ambiguities.  */
	   	// 如果我们在寻找未版本化的符号时只看到了一个版本化的符号，
		// 并且版本不是默认版本，我们仍然接受这个符号，因为没有可能的歧义。
		...
		found_it:
			...
			// 当UNDEF_MAP为NULL时，这表明我们是从do_lookup_x调用的，
			// 用于对受保护数据的重定位，我们跳过可执行文件中的数据定义。
				...
				&& type_class == ELF_RTYPE_CLASS_EXTERN_PROTECTED_DATA)
			{
				...
				if (map->l_info[DT_REL] != NULL && map->l_info[DT_RELSZ] != NULL && map->l_info[DT_RELSZ]->d_un.d_val != 0)
				{
					const ElfW(Rel) *rel = (const ElfW(Rel) *)D_PTR(map, l_info[DT_REL]);
					unsigned int rel_count = map->l_info[DT_RELSZ]->d_un.d_val / sizeof(*rel);

					for (i = 0; i < rel_count; i++, rel++)
						if (elf_machine_type_class(ELFW(R_TYPE)(rel->r_info)) == ELF_RTYPE_CLASS_COPY)
						{
			...
			switch (ELFW(ST_BIND)(sym->st_info))	// 符号绑定类型
			{
			case STB_WEAK:	// 若为弱符号
				/* Weak definition.  Use this value if we don't find another.  */
				// 弱符号。如果后续找不到其他定义，则使用此值。
...
add_dependency(struct link_map *undef_map, struct link_map *map, int flags)
{
	...
	struct link_map_reldeps *l_reldeps = atomic_forced_read(undef_map->l_reldeps);
	...
	if (__glibc_unlikely(flags & DL_LOOKUP_GSCOPE_LOCK))
	{
	   ...
	   changed in the mean time.  */
		if (undef_map->l_initfini != l_initfini && undef_map->l_initfini != NULL)
		{
	...
	if (runp != NULL)
	{
	   ...
	   definition.  */
		...
		{
			if (__glibc_unlikely(GLRO(dl_debug_mask) & DL_DEBUG_BINDINGS) && !is_nodelete(map, flags))
			{
...
marking %s [%lu] as NODELETE due to reference to %s [%lu]\n",
		...
		if (__glibc_unlikely(l_reldepsact >= undef_map->l_reldepsmax))
		{
			...
			unsigned int max = undef_map->l_reldepsmax ? undef_map->l_reldepsmax * 2 : 10;
...
/* 在加载的对象的符号表中搜索符号UNDEF_NAME的定义，可能是符号的请求版本。
   我们绝不能在这个函数中或在任何被调用的函数中调用审计函数。
   如果这样做，审计代码可能会创建一个线程，这可能会扰乱所有的作用域锁定。*/
lookup_t
...
{
	const uint_fast32_t new_hash = dl_new_hash(undef_name); // 根据符号名计算哈希值
	unsigned long int old_hash = 0xffffffff;
	struct sym_val current_value = {NULL, NULL}; // 保存最后的返回结果
	struct r_scope_elem **scope = symbol_scope;	// 保存符号的查找范围

	bump_num_relocations();

	/* DL_LOOKUP_RETURN_NEWEST does not make sense for versioned
	   lookups.  */
	// DL_LOOKUP_RETURN_NEWEST对于版本化查找没有意义
	...
	if (__glibc_unlikely(skip_map != NULL))
		...
		// 在相关的加载对象中搜索定义
		while ((*scope)->r_list[i] != skip_map)	// 跳过skip_map
			++i;

	/* Search the relevant loaded objects for a definition.  */
	// 在相关的加载对象中搜索定义
	for (size_t start = i; *scope != NULL; start = 0, ++scope)
		if (do_lookup_x(undef_name, new_hash, &old_hash, *ref,	// 查找符号
	...
	if (__glibc_unlikely(current_value.s == NULL))
	{
		if ((*ref == NULL || ELFW(ST_BIND)((*ref)->st_info) != STB_WEAK) && !(GLRO(dl_debug_mask) & DL_DEBUG_UNUSED))
		{
			...
			_dl_exception_create_format(&exception, DSO_FILENAME(reference_name),
	...
	int protected = (*ref && ELFW(ST_VISIBILITY)((*ref)->st_other) == STV_PROTECTED);	// 是否是保护符号
	if (__glibc_unlikely(protected != 0))
	{
		/* It is very tricky.  We need to figure out what value to
	   return for the protected symbol.  */
	   	// 这很棘手。我们需要弄清楚保护符号的返回值。
	...
	/* 我们必须检查这是否会将UNDEF_MAP绑定到动态加载的全局范围中的对象。
	   在这种情况下，我们必须防止后者被卸载，除非UNDEF_MAP对象也被卸载。*/
	if (__glibc_unlikely(current_value.m->l_type == lt_loaded)
		/* Don't do this for explicit lookups as opposed to implicit
	   runtime lookups.  */
	   	// 不要对显式查找进行此操作，而不是对隐式运行时查找进行此操作。
	   ...
	   was just removed.  Try finding another definition.  */
		// 出了点问题。也许我们试图引用的对象刚刚被删除。尝试找到另一个定义。
	...
	if (__glibc_unlikely(GLRO(dl_debug_mask) & (DL_DEBUG_BINDINGS | DL_DEBUG_PRELINK)))
		_dl_debug_bindings(undef_name, undef_map, ref,
						   &current_value, version, type_class, protected);

	*ref = current_value.s;
	return LOOKUP_VALUE(current_value.m);	// 返回查找结果
}
...
#error ELF_RTYPE_CLASS_COPY must be 0 or 2!
		...
		if ((GLRO(dl_trace_prelink_map) == NULL || GLRO(dl_trace_prelink_map) == GL(dl_ns)[LM_ID_BASE]._ns_loaded) && undef_map != GL(dl_ns)[LM_ID_BASE]._ns_loaded)
		{
			const uint_fast32_t new_hash = dl_new_hash(undef_name);
			unsigned long int old_hash = 0xffffffff;
			struct unique_sym *saved_entries = GL(dl_ns)[LM_ID_BASE]._ns_unique_sym_table.entries;

			GL(dl_ns)
			[LM_ID_BASE]._ns_unique_sym_table.entries = NULL;
			do_lookup_x(undef_name, new_hash, &old_hash, *ref, &val,
						undef_map->l_local_scope[0], 0, version, 0, NULL,
						type_class, undef_map);
			if (val.s != value->s || val.m != value->m)
				conflict = 1;
			else if (__glibc_unlikely(undef_map->l_symbolic_in_local_scope) && val.s && __glibc_unlikely(ELFW(ST_BIND)(val.s->st_info) == STB_GNU_UNIQUE))
			{
			   ...
			   library, it would be the one that is looked up.  */
				...
				for (n++; n < scope->r_nlist; n++)
					if (scope->r_list[n]->l_info[DT_SYMBOLIC] != NULL && do_lookup_x(undef_name, new_hash, &old_hash, *ref,
																					 &val2,
			...
			GL(dl_ns)
			[LM_ID_BASE]._ns_unique_sym_table.entries = saved_entries;
		}
		...
		if (value->s)
		{
			...
			else if (__glibc_unlikely(ELFW(ST_TYPE)(value->s->st_info) == STT_GNU_IFUNC))
				/* Set the RTYPE_CLASS_VALID bit.  */
				type_class |= RTYPE_CLASS_VALID;
		}

		if (conflict || GLRO(dl_trace_prelink_map) == undef_map || GLRO(dl_trace_prelink_map) == NULL || type_class >= 4)
		{
