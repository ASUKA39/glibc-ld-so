...
#define _ELF_DYNAMIC_DO_RELOC(RELOC, reloc, map, scope, do_lazy, skip_ifunc, test_rel)                 \
  ...
  {                                                                                                    \
    ...
    if ((map)->l_info[DT_PLTREL] && (!test_rel || (map)->l_info[DT_PLTREL]->d_un.d_val == DT_##RELOC)) \
    {                                                                                                  \
      ...
      if (ranges[0].start + ranges[0].size == (start + size))                                          \
        ranges[0].size -= size;                                                                        \
      if (!(do_lazy) && (ranges[0].start + ranges[0].size) == start)                                   \
      {                                                                                                \
...
/* ELF_DYNAMIC_RELOCATE 是一个宏，用于执行动态重定位的操作。
  map 是一个参数，表示动态链接器的映射信息，包括动态库的加载地址、动态段信息等。
  scope 是一个参数，表示重定位的范围。
  lazy 是一个参数，表示是否启用懒惰（lazy）重定位。
  consider_profile 是一个参数，表示是否考虑性能分析（profile）。
  skip_ifunc 是一个参数，表示是否跳过 IFUNC（indirect function）类型的重定位 */
/* 第一句调用 elf_machine_runtime_setup 函数，该函数执行与平台相关的运行时设置，并返回一个表示是否使用懒惰重定位的标志 edr_lazy
  第二句调用 ELF_DYNAMIC_DO_REL 宏，该宏执行 REL 类型的动态重定位。
  第三句调用 ELF_DYNAMIC_DO_RELA 宏，该宏执行 RELA 类型的动态重定位。RELA 是有加数版本的 REL */
#endif
