...
/* 这里的想法是，为了符合ABI，我们应该尝试在0x10000（我们实际上使用0x40000作为下限，
   以增加从空指针的内存引用导致段错误的机会）和程序的加载地址之间加载动态对象；
   这可能允许我们在PLT中使用分支指令而不是计算跳转。该地址仅用作mmap的首选项，
   因此如果我们弄错了，最糟糕的情况是它被映射到其他地方。 */
ElfW(Addr)
